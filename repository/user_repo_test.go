...
func (suite *UserRepositoryTestSuite) TestCreateUser_Success() {
	...
	suite.sqlmock.ExpectQuery("insert into users").WithArgs(dummy.Fullname, dummy.Role, dummy.Email, dummy.Password, sqlmock.AnyArg(), sqlmock.AnyArg(), dummy.IsDeleted).
		WillReturnRows(rows)
...
func (suite *UserRepositoryTestSuite) TestUpdateUser_Success() {
	...
	rows := sqlmock.NewRows([]string{"user_id", "fullname", "role", "email", "password", "created_at", "updated_at", "is_deleted"}).
		AddRow(dummy.UserID, dummy.Fullname, dummy.Role, dummy.Email, dummy.Password, dummy.CreatedAt, dummy.UpdatedAt, dummy.IsDeleted)
	suite.sqlmock.ExpectQuery(`update users set fullname \= \$1,role \= \$2,email \= \$3,password \= \$4, updated_at \= \$5, is_deleted \= \$6 where user_id \= \$7 returning user_id, fullname, role, email, password, created_at, updated_at, is_deleted`).WithArgs(dummy.Fullname, dummy.Role, dummy.Email, dummy.Password, sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg()).
		WillReturnRows(rows)
	suite.sqlmock.ExpectCommit()
